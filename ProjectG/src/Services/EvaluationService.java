...
public class EvaluationService {
    ...
    public int ajouterEvaluation(Evaluation e) {
        ...
        try {
            ...
            String req1 = "INSERT INTO `evaluation`( `contenu`,`rating` ,`id_personne`) VALUES ('" + e.getContenu() + "','" + e.getRating() + "','"+ e.getId_personne() + "')";
    ...
    public void modifierEvaluation(int idEvaluation, Evaluation e) throws SQLException {
        String update = "UPDATE `evaluation` set contenu =?, `rating` = ? , `id_personne` = ? where idEvaluation = '" + idEvaluation + "';";
        try {
            PreparedStatement st = x.prepareStatement(update);

            st.setString(1, e.getContenu());
            st.setInt(2, e.getRating());
            st.setInt(2, e.getId_personne());
...
//    } 
    public List<Evaluation> afficherEvaluation() throws SQLException {
        ...
        while (rst.next()) {
            ...
            ev.setId_personne(rst.getInt("rating"));
            ev.setId_personne(rst.getInt("id_personne"));
